...
class UserRegistrationSerializer(serializers.ModelSerializer):
  ...
  class Meta:
    model = User
    # fields=['email', 'first_name', 'last_name', 'country_code', 'phone_number', 'password', 'password2']
...
class SendPasswordResetEmailSerializer(serializers.Serializer):
  ...
  def validate(self, attrs):
    ...
    if User.objects.filter(email=email).exists():
      ...
      link = 'http://localhost:3000/password/user/reset/'+uid+'/'+token
...
class PhoneOTPVerificationSerializer(serializers.Serializer):
    ...
    def validate_otp(self, value):
        try:
            phone_otp = PhoneOTP.objects.get(otp=value)
        ...
        if not phone_otp.validated:
            phone_otp.validate()
            phone_otp.user.is_verified = True
            phone_otp.user.save()
